...
export default function Chat() {
    ...
    (async () => {
      ...
      try {
        ...
        if (text) {
          ...
          setInput(
            error ? `[Transcription error] ${error}` : "[No speech detected]",
          );
        }
      } catch (e: any) {
  ...
  return (
    <div className="min-h-screen flex flex-col bg-secondary text-white">
                ...
                {SUGGESTIONS.map((s) => (
                  <li key={s}>
                    <button
                      type="button"
                      onClick={() => setInput(s)}
                      className="rounded-full border border-white/10 bg-white/5 px-3 py-1.5 text-xs sm:text-sm hover:bg-white/10 disabled:opacity-50 transition"
                    >
                      {s}
                    </button>
                  </li>
      ...
      <form
        onSubmit={onSubmit}
        className="border-t border-white/10 bg-secondary sticky bottom-0"
      >
        <div className="mx-auto w-full max-w-3xl px-3 sm:px-4 py-2 sm:py-3 pb-[max(0.5rem,env(safe-area-inset-bottom))]">
          ...
          <div className="flex flex-wrap items-center gap-3 text-xs text-gray-400 mt-2">
            {rec.status === "recording" && (
              <span>Recording… {rec.seconds}s — tap ⏹ when done.</span>
            )}
